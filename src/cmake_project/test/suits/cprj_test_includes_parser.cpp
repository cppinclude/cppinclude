...
{
	//Run
	auto actualResult = parseString( "/bin/c++ -o f.cpp.o -c \\\"space in file\\\".cpp" );
	Strings exceptedResult;
...
BOOST_AUTO_TEST_CASE(t7_include_with_space)
{
	//Run
	auto actualResult =
		parseString( "/bin/c++ -o f.cpp.o -I\\\"./include with space\\\" -c f.cpp" );
...
{
	//Run
	auto actualResult = parseString( "/bin/c++  -I./include -o f.cpp.o -c f.cpp" );
	Strings exceptedResult = toPathStrings( {"./include"} );
