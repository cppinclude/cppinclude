...
{
	INTERNAL_CHECK_ERROR( m_cmakeProject );
...
void CMakeProjectBuilderFixture::parserArgument(
...
)
{
	...
	argumentsStr.push_back( "./application" );
	argumentsStr.push_back( std::string{ _argument } );
...
CMakeProjectBuilder & CMakeProjectBuilderFixture::ensureBuilder()
{
	if( !m_builder )
	{
		m_builder.reset( new CMakeProjectBuilder{
			ensureCMakeAccessor(),
...
{
	if( !m_fsAccessor )
		m_fsAccessor.reset( new fs::FileSystemAccessorImpl );
...
{
	if( !m_cmakeAccessor )
		m_cmakeAccessor.reset( new cmake_project::AccessorImpl );
...
compilation_db::Accessor & CMakeProjectBuilderFixture::ensureCompilationDbAccessor()
{
	if( !m_compilationDbAccessor )
		m_compilationDbAccessor.reset( new compilation_db::AccessorImpl );
...
{
	if( !m_jsonAccessor )
		m_jsonAccessor.reset( new json::JsonAccesorImpl );
