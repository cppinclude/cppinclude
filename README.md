...
and *base_char.hpp* will be recompiled and it will take time. This tool helps to find
the file at the top of the include hierarchy:
...
Path to a folder that contains sources. Often source files are located in *src* or
*sources* folder, not in the root folder of the project.    
...
If you use other file extensions than *.cpp and *.h for C++ sources you can specify them in the configuration file with:
...
Given a *main.cpp* file and a *include* folder that stores the *header.hpp* header:
...
Show headers that are found in the search directories but that were not included in the source code.
It often happens after refactoring when headers when include directives were removed from code and the files remained in place.
...
If headers have the same name but are located in different paths only one occurrence will be counted.
...
It helps to follow code styles in projects,
