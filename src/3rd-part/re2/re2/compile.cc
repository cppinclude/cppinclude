...
Frag Compiler::PostVisit(Regexp* re, Frag, Frag, Frag* child_frags,
  ...
  switch (re->op()) {
    ...
    case kRegexpStar:
	  return Star(child_frags[0], (re->parse_flags()&Regexp::NonGreedy) != 0); // NOLINT

    case kRegexpPlus:
	  return Plus(child_frags[0], (re->parse_flags()&Regexp::NonGreedy) != 0); // NOLINT

    case kRegexpQuest:
	  return Quest(child_frags[0], (re->parse_flags()&Regexp::NonGreedy) != 0); // NOLINT
    ...
    case kRegexpCapture:
      ...
      if (re->cap() < 0)
		return child_frags[0]; // NOLINT
	  return Capture(child_frags[0], re->cap()); // NOLINT
